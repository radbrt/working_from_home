...
jobs %>% 
  mutate(hk = grepl("hjemme", stillingsbeskrivelse_vasket, ignore.case=T )) %>% 
  select(isco_versjon, yrkeskode, yrke, hk, arbeidssted_kommunenummer, statistikk_aar_mnd) -> hjemme
